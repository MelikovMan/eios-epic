...
### 2.1 Архитектура Redux

Основаная идея Redux - сделать состояние глобальным немутриующим объектом, к которому могут обращаться компоненты.

Эти компоненты с помощью вызова функций reducer, хранящий полезные данные в action, пересоздает это хранилище данных в зависимости от reducer и action

В результате получается односторонний поток данных

![Поток данных Redux](/data_flow.jpg)

### 2.2. Современный Redux - не ваш дедушкин Redux
```jsx
// store/store.js
...
```
### 2.3. Использование в компонентах с хуками
```jsx
// hooks/redux.js
...
```
### 2.4. Асинхронные действия с createAsyncThunk
```jsx
...
```
### 2.5. RTK Query для работы с API
```jsx
// store/api.js
