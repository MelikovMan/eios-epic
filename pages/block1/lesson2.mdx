...
Другими словами, можно включать лишь те элементы языка, которые однозначно возвращают выражение без побочных эффектов
...
Ключи (key) обязательны для элементов списка, алгоритм диффинга действует из предположения, что это так, иначе перестройка списка будет очень дорогостоящей операцией!
